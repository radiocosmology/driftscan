...
class CombineBeamTransfers(tasklib.base.ContainerTask):
    ...
    def process(self, bt_full):
        ...
        from ..core.telescope import PolarisedTelescope

        if isinstance(bt_full, BeamTransfer):
            self.beamtransfer_full = bt_full
        elif isinstance(bt_full, manager.ProductManager):
            self.beamtransfer_full = bt_full.beamtransfer
        else:
            raise RuntimeError(
                "bt_full is not a BeamTransfer or ProductManager instance"
            )

        self.telescope_full = bt_full.telescope
        ...
        for mi in mpitools.mpirange(self.telescope_full.mmax + 1, method="alt"):
            ...
            for i in range(self.n_partials):
                ...
                if mi <= self.telescope_partial[i].mmax:
                    with h5py.File(
                        ...
                        # First load BTM elements from partial-BTM file, then write to
                        # new file, to avoid broadcasting error for complex selections.
                        # If copying specific polarizations, set unwanted pols to zero
                        # in the process.
